...
def serializationFilter(obj, exclude: list[str] = []):
    def should_exclude(key):
        key_str = str(key)
        return any(ex in key_str for ex in exclude)
    
    if isinstance(obj, Enum):
        return obj.name
    elif isinstance(obj, dict):  
        return {
            str(k): serializationFilter(v, exclude) for k, v in obj.items() if not str(k).startswith("_") and not should_exclude(k)
        }
    elif hasattr(obj, "__dict__"):
        return {
            str(k): serializationFilter(v, exclude) for k, v in vars(obj).items() if not str(k).startswith("_") and  not should_exclude(k)
        }
